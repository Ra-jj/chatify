    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-base-200 w-full max-w-md rounded-2xl shadow-xl flex flex-col max-h-[90vh]">
          ...
          <div className="form-control mt-2">
            ...
            <div className="flex flex-col gap-2 max-h-48 overflow-y-auto pr-2">
              {filteredUsers.map(user => (
                <label key={user._id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-base-300 cursor-pointer transition-colors border border-transparent hover:border-base-300">
                  <input 
                    type="checkbox" 
        ...
        <div className="p-5 border-t border-base-300 flex justify-end gap-3 bg-base-300/50 rounded-b-2xl">
          ...
          >
            {isLoading ? <span className="loading loading-spinner loading-sm"></span> : "Create Group"}
          </button>
        </div>
      </div>
    </div>
